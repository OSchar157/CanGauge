...
class RPMGaugeWidget(QWidget):
    ...
    def _draw_arc(self, painter):
        ...
        # Normal zone
        normal_span = self._value_to_angle_span(
            self.warn_val if self.warn_val else self.max_val
        )
        ...
        if self.warn_val:
            warn_span = self._value_to_angle_span(
                self.danger_val if self.danger_val else self.max_val
            ) - normal_span
            pen.setColor(QColor("#E8A020"))
            painter.setPen(pen)
            painter.drawArc(-78, -78, 156, 156,
                            int((start_angle - normal_span) * 16),
                            -int(warn_span * 16))

        if self.danger_val:
            danger_start_span = self._value_to_angle_span(self.danger_val)
            danger_span = span - danger_start_span
            pen.setColor(QColor("#CC2200"))
            painter.setPen(pen)
            painter.drawArc(-78, -78, 156, 156,
                            int((start_angle - danger_start_span) * 16),
                            -int(danger_span * 16))

    def _draw_ticks(self, painter):
        num_major = 8
        num_minor = 4  # minor ticks between each major
        ...
        for i in range(total_ticks + 1):
            ...
            if self.danger_val and val >= self.danger_val:
                color = QColor("#CC2200")
            elif self.warn_val and val >= self.warn_val:
                color = QColor("#E8A020")
            else:
                color = QColor("#ffffff")
    ...
    def _draw_unit(self, painter):
        ...
        if self.max_val >= 1000:
            ...
            font2 = QFont("Courier New", 4)
            painter.setFont(font2)
            painter.drawText(-20, 44, 40, 10, Qt.AlignCenter, "x1000")
...
class DemoWindow(QWidget):
    ...
    def _animate(self):
        self._demo_value += 40 * self._direction
        if self._demo_value >= 8000:
            self._direction = -1
